    ...
    def receive(self):
        while True:
            ...
            while nickname in self.clients.values():
                message = f'409 {nickname} is already taken'
                client.send(message.encode())
                print(f'the nickname = {nickname} ')
            
                received_nickname = client.recv(1024).decode('ascii')
                nickname_split = received_nickname.split(":")
                nickname = nickname_split[0]
